...
const codeExamples = {
  ...
  print(response.text)`,
    ...
    Shell: `curl --location '/api/file_to_order' \\
  ...
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, "/api/file_to_order");
  ...
  url = URI.parse("/api/file_to_order")
...
const AttachFileToOrder = () => {
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(setApiData({
      codeExamples,
      endpoint: "/api/file_to_order",
      method: "POST"
    }));
  }, [dispatch]);
  ...
  return (
    <div className="api-container">
      <h1 className="api-title">Добавление файл к ордеру(Attach File To Order)</h1>
      
      <div className="api-method-container">
        <span className="api-method post">POST</span>
        <span className="api-endpoint">/api/file_to_order</span>
      </div>

      <p className="api-description">
        Метод позволяет добавить файл к заказу.
        <br />
        <br />
...
export default AttachFileToOrder;
